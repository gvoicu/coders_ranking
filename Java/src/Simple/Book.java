    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (rs.next()) {
                ...
                // verificam daca cartea a mai fost ceruta de catre un alt user
                if(rs2.next()) {
                    id_name = rs2.getInt("id");
                }
                else {
                    query = "INSERT INTO `books` VALUES (NUll,'"+ name +"')";
                ...
                query = "UPDATE `user_info` SET `id_amazon`='"+ id_name +"' WHERE `email`='"+email+"'";
